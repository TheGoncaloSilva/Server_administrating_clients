...
def initiate_server(ip, port, max_hosts, name, enc, password):
    ...
    try: # Try server creatio 
        server_socket.bind((ip, port))
    except:
        print('ERROR: Address already in use 2')
        return False # Server criation failed
    
    server_socket.listen(max_hosts)  # max backlog of connections
    global s_sock
